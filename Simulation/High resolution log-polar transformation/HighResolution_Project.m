...
Wavelength = 405*nm; 
%% Light Source
E1 = 0*GenerateLGLightZheng(2, 0, 1.5*mm, var.rho, var.theta);
E2 = 1*GenerateLGLightZheng(1, 0, 1.5*mm, var.rho, var.theta);
E3 = 0*GenerateLGLightZheng(0, 0, 1.5*mm, var.rho, var.theta);
E4 = 0*GenerateLGLightZheng(-1, 0, 1.5*mm, var.rho, var.theta);
E5 = 0*GenerateLGLightZheng(-2, 0, 1.5*mm, var.rho, var.theta);

EG = exp(1i*rand(2048)*R1(3));              % 表面起伏
LightSource = E1 + E2 + E3 + E4 + E5; 
[xf, yf, Ef] = log_polar4(fftshift(LightSource));
%% Object
L = px*X_num;
N = X_num;
alpha = pi/4;
...
num = 3;
len = 0;
Filter = DeviatedFanFilter(L, N, num, alpha, theta, len);
LightSource = LightSource .* fftshift(Filter); 
LightSource = LightSource + En;
N = real(sum(sum(LightSource.*conj(LightSource)))/sum(sum(En.*conj(En))))
%%
...
% DrawIntenWithoutCorr(resize(Intensity(fftshift(rot90(Field))), 400),1)
%DrawEIntenAndAngle(resize(fftshift(Field), 100),1)
I = resize(Intensity(fftshift(rot90(Field))), 100);
b = CountingAngularSpectrum(I);
a = -4:4;
figure(2)
P = b/sum(b);
bar(a, P, 0.4)
